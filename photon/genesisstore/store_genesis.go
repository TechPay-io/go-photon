...
func (s *Store) GetRules() photon.Rules {
	cfg := s.rlp.Get(s.table.Rules, []byte("c"), &photon.Rules{}).(*photon.Rules)
	return *cfg
}

func (s *Store) SetRules(cfg photon.Rules) {
	s.rlp.Set(s.table.Rules, []byte("c"), &cfg)
}

func (s *Store) GetGenesis() photon.Genesis {
	meatadata := s.GetMetadata()
	return photon.Genesis{
