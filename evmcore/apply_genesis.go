...
func ApplyGenesis(statedb *state.StateDB, g photon.Genesis, maxMemoryUsage int) (*EvmBlock, error) {
	mem := 0
...
func genesisBlock(g photon.Genesis, root common.Hash) *EvmBlock {
	block := &EvmBlock{
		EvmHeader: EvmHeader{
			Number:   big.NewInt(0),
			Time:     g.Time,
...
func MustApplyGenesis(g photon.Genesis, statedb *state.StateDB, maxMemoryUsage int) *EvmBlock {
