...
import (
	...
	"github.com/TechPay-io/go-photon/cmd/cmdtest"
)

func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "photon-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "photon-test" in exec().
	reexec.Register("photon-test", func() {
...
func exec(t *testing.T, args ...string) *testcli {
	...
	// Boot "photon". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("photon-test", args...)
