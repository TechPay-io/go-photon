...
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../photon-sfc:/src -w /src go-photon-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; sed -i s/runs:\\ 200,/runs:\\ 10000,/ /src/truffle-config.js; rm -f /src/build/contracts/*json; npm run build'"
//go:generate bash -c "cd ../../photon-sfc && git checkout -- truffle-config.js; docker rmi go-photon-sfc-compiler-image"
...
import (
	...
	"github.com/TechPay-io/go-photon/gossip/contract/driver100"
	"github.com/TechPay-io/go-photon/gossip/contract/driverauth100"
	"github.com/TechPay-io/go-photon/gossip/contract/netinit100"
	"github.com/TechPay-io/go-photon/gossip/contract/sfc100"
	"github.com/TechPay-io/go-photon/logger"
	"github.com/TechPay-io/go-photon/photon/genesis/driver"
	"github.com/TechPay-io/go-photon/photon/genesis/driverauth"
	"github.com/TechPay-io/go-photon/photon/genesis/evmwriter"
	"github.com/TechPay-io/go-photon/photon/genesis/netinit"
	"github.com/TechPay-io/go-photon/photon/genesis/sfc"
...
func TestSFC(t *testing.T) {
	...
	_ = true &&
		...
		t.Run("SFC upgrade", func(t *testing.T) {
			...
			rr := env.ApplyBlock(nextEpoch,
				env.Contract(admin, utils.ToTpc(0), sfc100.ContractBin),
			)
	...
	t.Run("Direct driver", func(t *testing.T) {
		...
		rr := env.ApplyBlock(nextEpoch,
			env.Contract(admin, utils.ToTpc(0), anyContractBin),
		)
		require.Equal(1, rr.Len())
		require.Equal(types.ReceiptStatusSuccessful, rr[0].Status)
		newImpl := rr[0].ContractAddress
...
func cicleTransfers(t *testing.T, env *testEnv, count uint64) {
	...
	for i := uint64(0); i < count; i++ {
		...
		for i := range txs {
			from := (i)%accounts + 1
			to := (i+1)%accounts + 1
			txs[i] = env.Transfer(from, to, utils.ToTpc(100))
		}
